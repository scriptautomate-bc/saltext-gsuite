...
def _get_pydir(session):
    ...
    if version_info < (3, 14):
        session.error("Only Python >= 3.14 is supported")
...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/gsuite/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/gsuite/*")
            ...
            #    "--include=src/saltext/gsuite/*,tests/*"
            # )
        finally:
