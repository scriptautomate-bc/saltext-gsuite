...
linkcheck_ignore = [
    r"http://localhost:\d+",
    # Changelog entries reference their originating issue/PR number, but those URLs
    # 404 until the issue/PR is actually opened on GitHub, so skip the issue tracker.
    r"https://github\.com/salt-extensions/saltext-gsuite/issues/\d+",
    # The generated module reference pages embed Google API doc links lifted verbatim
    # from the Discovery descriptions. Google reorganizes those pages frequently
    # (renamed/removed -> 404, shifted anchors) and some descriptions wrap the URL
    # mid-string (yielding truncated links). They are autogenerated and not
    # hand-maintainable, so skip the Google vendor-doc domains during linkcheck.
    r"https://developers\.google\.com/.*",
    r"https://cloud\.google\.com/.*",
    r"https://support\.google\.com/.*",
    r"https://www\.googleapis\.com/auth/.*",
    # Truncated/malformed URLs from line-wrapped docstrings (e.g. "https://w",
    # "https://developers.google.co") - a scheme with only a partial host, no path.
    r"https:/{1,2}[^/]*$",
]
